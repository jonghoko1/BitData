...
// 스크롤바 설정
document.getElementById('description').style.overflowY = 'auto';

// memo-update-modal.js 파일 내용

document.addEventListener('DOMContentLoaded', function() {
    const memoUpdateModal = document.querySelector('.modal.memo-update');
    const memoUpdateBtns = document.querySelectorAll('.image-memo-btn');
    const titleInput = document.getElementById('title');
    const descriptionTextarea = document.getElementById('description');
    const saveBtn = document.querySelector('.save-btn');
    const closeBtn = document.querySelector('.modal.memo-update .close-btn');

// 이미지 메모 버튼 클릭 시 메모 수정 모달 열기
    memoUpdateBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            memoUpdateModal.style.display = 'block';

            // 예시: 기존 메모 내용을 가져와서 입력란에 표시
            const memoTitle = this.closest('tr').querySelector('.title').textContent;
            const memoDescription = this.closest('tr').querySelector('.description').textContent;

            titleInput.value = memoTitle;
            descriptionTextarea.value = memoDescription;
        });
    });
